...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, device, lr):
    ...
    for epoch in range(num_epochs):
        ...
        train_acc.append(train_accuracy["accuracy"])
...
def hyper_param(lr_list, num_epochs_list, model_name, batch):
  best_model = "model"
  best_acc = 0
  best_epoch = 0
  best_lr = 0
  for lr in lr_list:
    for num_epochs in num_epochs_list:
      pretrained_model, train_dataloader, validation_dataloader, test_dataloader = pre_process(model_name, batch, "cuda", False)
      train_acc, val_acc = train(pretrained_model, num_epochs, train_dataloader, validation_dataloader, "cuda", lr)
      if best_acc < val_acc[-1]:
        best_acc = val_acc[-1]
        best_model = pretrained_model
        best_lr = lr
        best_epoch = num_epochs
        print(evaluate_model(best_model, test_dataloader, "cuda"))
        print(best_lr)
        print(best_epoch)
  print("FINAL MODEL")
  print(evaluate_model(best_model, test_dataloader, "cuda"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hyperparam", type=bool, default=False)
    ...
    # load the data and models
        ...
        best_model = hyper_param(lr_list, num_epochs_list, args.model, args.batch_size)